...
def main():
    ...
    if args.client:
        ...
        for i in range(50):
            ...
            img.save(f'images/test_{i}.jpg')
            prompt = "In: What action should the robot take to pick hotdog?\nOut:"
            
            # Predict Action (7-DoF; un-normalize for BridgeData V2)
            inputs = processor(prompt, img).to("cuda:0", dtype=torch.bfloat16)
            action = vla.predict_action(**inputs, unnorm_key="bridge_orig", do_sample=False)
            print("Predicted Action:", action)
            res = widowx_client.step_action(action)
            time.sleep(1)
        
        # print(obs.keys, obs)
        # print(type(obs['full_image']))
        ...
        # move left up
        # res = widowx_client.move(np.array([0, 0.0, 0.2, 0, 0, 0]))
        # print(res)
